...
public class OperationsManager{
	private UI ui;
	//private ArrayList<ArrayList<BigDecimal>> matrix = new ArrayList<ArrayList<BigDecimal>>();
	
	public OperationsManager(UI ui) {
		//to use UI functions from here
		this.ui = ui;
	}
	
	//multiplies a row in a matrix of type double by value
	public void multiplyRow(int row, double value, ArrayList<ArrayList<Double>> matrix){
		if(value % 1 == 0){
		...
		for(double d : matrix.get(row-1)){
			...
			//matrix.get(row-1).set(i, d.multiply(value));
			i++;
		}
	...
	public void addMultipleOfRowToRow(int row1, int row2, double value, ArrayList<ArrayList<Double>> matrix){
		int i = 0;
	...
	public void swapRowWithRow(int row1, int row2, ArrayList<ArrayList<Double>> matrix){
	...
	public boolean isAllZero(int row, ArrayList<ArrayList<Double>> matrix){
	...
	public void reduceMatrix(ArrayList<ArrayList<Double>> matrix){
		...
		for(int i = 0; i < matrix.size(); i++){
			...
			if(i+2 < matrix.size()){
				if(matrix.get(i).get(i) == 0 && !isAllZero(i+2, matrix)){
					swapRowWithRow(i+1, i+2, matrix);
				}
			}
			...
			if(matrix.get(i).get(i) != 1 && matrix.get(i).get(i) != 0){	
				...
				if(matrix.get(i).get(i) != 1){
					multiplyRow(i+1, 1 / matrix.get(i).get(i), matrix);
				}
			...
			for(int k = 0; k < matrix.size(); k++){
				if(matrix.get(k).get(i) != 0 && k != i && matrix.get(i).get(i) != 0){	
					addMultipleOfRowToRow(i+1, k+1, matrix.get(k).get(i) * -1, matrix);
				}		
			}	
		}
		...
		for(int i = matrix.size(); i > 1; i--){
			if(isAllZero(i-1, matrix)){
				swapRowWithRow(i, i-1, matrix);
			}
		}
		...
		for(int i = 0; i < matrix.size(); i++){
			for(int j = 0; j < matrix.get(i).size(); j++){
				if(matrix.get(i).get(j) == 1){
					break;
				}else if(!isAllZero(i+1, matrix) && matrix.get(i).get(j) != 0){
					multiplyRow(i+1, 1 / matrix.get(i).get(j) , matrix);
					for(int k = 0; k < matrix.size(); k++){
						if(matrix.get(k).get(j) != 0 && matrix.get(k).get(j) != 1){	
							addMultipleOfRowToRow(i+1, k+1, matrix.get(k).get(j) * -1, matrix);
						}		
					}
				}		
			}
